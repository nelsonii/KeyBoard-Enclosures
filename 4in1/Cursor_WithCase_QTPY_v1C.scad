...
module QTPY() {
  //riser for QT PY
    color("teal") translate([(caseWidthX/2)-6, 2, 1]) cube([12,21.5,1.5]);
  //bumper for QT PY
  color("CadetBlue") translate([(caseWidthX/2)-6, 23, 1]) cube([12, 2, 5]);
}

module QTPY_Cutout() {
      //QT PY USB -- 1.65 is thickness of the qtpy pcb
    color("peru") translate([(caseWidthX/2)-5, -1, 3+1.65]) cube([10,8,3.50]); 
}
